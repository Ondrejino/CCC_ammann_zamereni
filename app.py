...
if uploaded_file is not None:
    ...
    if len(df) < 5:
        st.error("Málo platných dat pro výpočet.")
    else:
        geod = Geod(ellps="WGS84")
        
        # --- VYHLAZENÍ ŠUMU GPS (Řešení rotujícího válce) ---
        window = 5
        ...
        if show_heatmap:
            ...
            fig_heat = go.Figure()
            df_h = df.iloc[::decim_heatmap]
            
            fig_heat.add_trace(go.Scatter(
                x=df_h['corr_lon'], y=df_h['corr_lat'], mode='markers',
                marker=dict(size=6, color=df_h[col_stiff], colorscale='Jet', showscale=True, colorbar=dict(title="Kb [-]")),
        ...
        if not in_circle.empty:
            ...
            fig_line.add_trace(go.Scatter(
                x=summary['Pojezd'], y=summary['Kb [-]'], 
                mode='markers+lines', marker=dict(size=10, color='royalblue'), line=dict(width=3)
            ))
            fig_line.update_layout(xaxis_title="Číslo pojezdu", yaxis_title="Průměrná hodnota Kb [-]", xaxis=dict(dtick=1))
